...
tests/NekoClicker.Core.Tests/    210 个测试 + 自研迷你测试运行器（含架构不变量与全程可达测试）
...
- 核心引擎、三个内容包（猫咖物语 / 猫娘咖啡馆 / 九命轮回）、终端 Demo、**210 个测试**全部通过。
...
- **图鉴 / 叙事释放（S-B）已落地**：叙事线 + 条目、条件达成即解锁、图鉴面板（`???` 遮蔽
  ...
  全部条目的释放条件互不相同、且线内顺序与实际解锁顺序一致（见 `docs/CONTENT_AUTHORING.md` §12.1）。
...
- **未包含**：图形前端、本地化资源系统、账号/云存档、排行榜、反作弊，以及 ROADMAP 阶段 3~5
  的 `Choice` / `Decay` 两项引擎能力。这些都被设计为引擎外部的宿主职责或后续阶段。
