...
- 六个内容包（#1 咖啡馆 / #2 九命 / #3 实验室 / #10 公司 / #6 末世 / #9 图书馆）的数值曲线
  ...
  #6 与 #9 跑 12 小时看阶段推进）。
- **一个已知的跨包问题（尚未修）**：纪元包（#2/#3/#10/#6/#9）各跑 48 小时后
  「当前等级 / 转生货币」都是 0——等级只在舍命那一刻结算，而"够条件就走"的机器人
  永远在等级 0 时舍命，于是「前世技能 / 前世经验 / 余烬 / 批注」这一整条线在自然游玩里打不开。
  它并非死内容（末世包里实测**在第 4 层多攒 0.9 游戏小时**就能把等级顶到 1，
  见 `ApocalypseContentTests.StoringUpBeforeTheLastRestart_IsAffordable`），
  但各包的阈值与提示都没有把这条路径讲清楚，留待专项处理。
