...
**零第三方依赖**，附带六个内容包（示例包「猫咖物语」+ #1《猫娘咖啡馆》+ #2《九命轮回》
+ #3《猫娘实验室》+ #10《猫娘公司》+ #6《猫娘末世》）和一个可玩的终端 Demo。
...
# 末世没有表态面板（唯一不需要选择轴的包）：看它的图鉴与纪元总览
...
tests/NekoClicker.Core.Tests/    305 个测试 + 自研迷你测试运行器（含架构不变量与全程可达测试）
...
docs/NINE_LIVES_DESIGN.md        《九命猫娘》设计映射：1 个共享核心 + 10 个内容包（5 个已落地）
...
- 核心引擎、六个内容包（猫咖物语 / 猫娘咖啡馆 / 九命轮回 / 猫娘实验室 / 猫娘公司 / 猫娘末世）、
  终端 Demo，**305 个测试**全部通过。
- **分层转生（`Era`）已落地**：逐级推进的转生按钮、每层换规则的平衡覆盖、
  跨层继承、构建期的完成条件单调性校验。九命（9 层）、实验室（7 批）、公司（3 轮）、
  末世（5 次重启）全程可达由机器人测试守住。
- **图鉴 / 叙事释放（S-B）已落地**：叙事线 + 条目、条件达成即解锁、图鉴面板（`???` 遮蔽
  与条件进度）、待读提示、存档往返。#1 咖啡馆 51 条（3 条线）+ #2 九命 50 条（4 条线）
  + #3 实验室 40 条（4 条线）+ #10 公司 40 条（4 条线）+ #6 末世 40 条（4 条线），全部条目的释放条件互不相同、
  且线内顺序与实际解锁顺序一致（见 `docs/CONTENT_AUTHORING.md` §12.1）。
...
- **阶段 4（前半）已交付**：内容包 #6《猫娘末世》——**全项目第一个用「跨转生继承」的包**
  （继承比例 0 → 25% → 40% → 55% → 70%，每次重启能多带走一点），第二资源「记忆残片」
  的产率取决于上一次重启留下了多少座位，三个结局由"记住了多少 + 图鉴读了多少"决定
  （与另外四个包的立场驱动结局共用同一套 `EndingDefinition`）。
  它同时暴露并处理了一个继承特有的坑：解锁条件若用「本轮累计」，重启归零之后，
  手上已有的建筑会显示成「未解锁」——这个包改用「历史累计」，并有用例钉住。
  S-D（虚无化）与 #9《猫娘图书馆》、#4/#5/#7/#8 四个换皮包仍未开始。
- 五个内容包（#1 咖啡馆 / #2 九命 / #3 实验室 / #10 公司 / #6 末世）的数值曲线都经过无头模拟验证
  （见 `--simulate --auto`；#2 建议跑 48 小时、#3 跑 4 小时、#10 跑 6 小时、#6 跑 12 小时看阶段推进）。
- **一个已知的跨包问题（尚未修）**：四个纪元包（#2/#3/#10/#6）各跑 48 小时后
  「当前等级 / 转生货币」都是 0——等级只在舍命那一刻结算，而"够条件就走"的机器人
  永远在等级 0 时舍命，于是「前世技能 / 前世经验 / 余烬」这一整条线在自然游玩里打不开。
  它并非死内容（末世包里实测**在第 4 层多攒 2.0 游戏小时**就能把等级顶到 1，
  见 `ApocalypseContentTests.StoringUpBeforeTheLastRestart_IsAffordable`），
  但四个包的阈值与提示都没有把这条路径讲清楚，留待专项处理。
- **未包含**：图形前端、本地化资源系统、账号/云存档、排行榜、反作弊，以及 ROADMAP 阶段 4 后半
  的 `Decay` 引擎能力与 #9 图书馆、阶段 5 的 #4/#5/#7/#8 等内容包。
