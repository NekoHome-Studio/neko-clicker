...
**零第三方依赖**，附带四个内容包（示例包「猫咖物语」+ #1《猫娘咖啡馆》+ #2《九命轮回》
+ #3《猫娘实验室》）和一个可玩的终端 Demo。
...
tests/NekoClicker.Core.Tests/    265 个测试 + 自研迷你测试运行器（含架构不变量与全程可达测试）
...
docs/NINE_LIVES_DESIGN.md        《九命猫娘》设计映射：1 个共享核心 + 10 个内容包（3 个已落地）
...
- 核心引擎、四个内容包（猫咖物语 / 猫娘咖啡馆 / 九命轮回 / 猫娘实验室）、终端 Demo，
  **265 个测试**全部通过。
- **分层转生（`Era`）已落地**：逐级推进的舍命按钮、每层换规则的平衡覆盖、
  跨层继承、构建期的完成条件单调性校验。九命（9 层）与实验室（7 批）全程可达由机器人测试守住。
- **图鉴 / 叙事释放（S-B）已落地**：叙事线 + 条目、条件达成即解锁、图鉴面板（`???` 遮蔽
  与条件进度）、待读提示、存档往返。#1 咖啡馆 51 条（3 条线）+ #2 九命 50 条（4 条线）
  + #3 实验室 40 条（4 条线），全部条目的释放条件互不相同、且线内顺序与实际解锁顺序一致
  （见 `docs/CONTENT_AUTHORING.md` §12.1）。
- **选择 / 立场轴（S-C）与终局判定已落地**：内容自定义的 `StanceDefinition`（不是 enum）、
  作答后累加立场权重、主导立场作为 `ModifierResolver` 的第 5 个来源、末层按条件树挑出唯一结局。
  #2 九命（神性 / 人形 / 猫形 / 断绝，6 次表态，5 个结局）与 #3 实验室（乌托邦 / 叛乱 /
  共存 / 删除，6 次表态，5 个结局）共用同一套代码；Demo 有「表态」面板，无头报告会打印
  立场轴与结局（供 CI 断言）。立场与已答选择跨舍命与转生保留。
...
- 三个内容包（#1 咖啡馆 / #2 九命 / #3 实验室）的数值曲线都经过无头模拟验证
  （见 `--simulate --auto`；#2 建议跑 48 小时、#3 跑 4 小时看纪元推进）。
- **未包含**：图形前端、本地化资源系统、账号/云存档、排行榜、反作弊，以及 ROADMAP 阶段 4~5
  的 `Decay` 引擎能力与 #10 公司、#4~#9 等内容包。这些都被设计为引擎外部的宿主职责或后续阶段。
