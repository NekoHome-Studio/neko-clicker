...
**零第三方依赖**，附带五个内容包（示例包「猫咖物语」+ #1《猫娘咖啡馆》+ #2《九命轮回》
+ #3《猫娘实验室》+ #10《猫娘公司》）和一个可玩的终端 Demo。
...
docs/NINE_LIVES_DESIGN.md        《九命猫娘》设计映射：1 个共享核心 + 10 个内容包（4 个已落地）
...
- **图鉴 / 叙事释放（S-B）已落地**：叙事线 + 条目、条件达成即解锁、图鉴面板（`???` 遮蔽
  ...
  + #3 实验室 40 条（4 条线）+ #10 公司 40 条（4 条线），全部条目的释放条件互不相同、
...
- 四个内容包（#1 咖啡馆 / #2 九命 / #3 实验室 / #10 公司）的数值曲线都经过无头模拟验证
  （见 `--simulate --auto`；#2 建议跑 48 小时、#3 跑 4 小时、#10 跑 6 小时看阶段推进）。
- **未包含**：图形前端、本地化资源系统、账号/云存档、排行榜、反作弊，以及 ROADMAP 阶段 4~5
  的 `Decay` 引擎能力与 #4~#9 等内容包。这些都被设计为引擎外部的宿主职责或后续阶段。
