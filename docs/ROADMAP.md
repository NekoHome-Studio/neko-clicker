...
**引擎部分已交付**（`EndingDefinition` + `EndingSystem` + 状态/存档/事件/视图/构建期校验，
`EndingTests` 13 条）；**九命的内容部分（立场轴、选择、四个结局的文本）待做**。

已定下的判定语义：

- 判定**完全由条件树驱动**，引擎不认识"哪一层是最后一层"——"走完主线"写成 `EraAtLeast(9)`。
  这是 A2 的延续。
- **互斥**靠 `Priority`：升序取第一个满足条件的，记下之后判定永久停止。
  立场权重与图鉴条数都是单调不减的，达成后会一直成立，所以必须"判一次就停"。
- **兜底结局是构建期强制的**：至少有一个结局的条件里不含 `OwnedKind.Choice` 与
  `NumericMetric.StanceWeight`，否则回避表态的玩家会走完主线却没有结局。这就是验收 ③ 的机器化。
- 结局**只写** `Counters["ending_<id>"]` 与 `EndingsReached`，不重置存档（验收 ②）。

**内容部分待做，且有一个已知的设计取舍要面对**：一份存档只有一个结局，而立场权重跨转生保留，
所以探索其它结局必须开新存档。做内容时要确认这个取舍可接受——如果不接受，
就得让立场权重在转生时衰减或可重置，那是另一套机制。
