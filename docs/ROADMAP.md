...
| G3 | 转生分层（`Era`）能承载至少三种完全不同的叙事 | #2 九命 / #7 神明 / #10 公司 三个包的层定义共用同一套代码（当前 #2 九层、#3 实验室七批已共用这套代码，即已满两种；第三种待 #10 或 #7） |
| G4 | 玩家从第 1 层到最后一层**全程可达**，不存在卡死 | 机器人测试：脚本化自动游玩必须走到末层并触发终局 |
| G5 | 剧情随解锁释放，不一次讲完 | 图鉴覆盖率测试 + 前 10 分钟只释放 ≤3 条 |
| G6 | 每次改动后既有测试全绿（当前 265 个） | `.\tools\build.ps1` 退出码 0 |
...
| 特有验收 | 立场轴＝劳资，与 #3 的道德轴共用 3A/3B；核心仍不得新增任何与立场/选择相关的分支（A1/A2 的检验） |
|---|---|
...
- 阶段 0（#1 猫娘咖啡馆 + 架构测试 + `--package`）、阶段 1（S-A `Era` + #2 九命轮回）、
  ...
  立场轴 + 终局判定 + 九命四条立场）与 3C-1（#3 实验室）均已落地。
  十个包里 #1 / #2 / #3 已可玩，示例包「猫咖物语」作为回归基线保留。
  **S-D `Decay` 一行未写**；#10 公司与 #4~#9 仍只是文档。
- 阶段 4 的 S-D 接口**有意留白**，因为它的正确形式依赖 #9 包的实际节奏，现在设计等于猜。
- 阶段 5 的四个"换皮"包能批量做，前提是阶段 1~2 的抽象确实成立——**该前提已经用 #3 验证过一次**
  （同一个核心跑 Era + Lore + Choice 三套能力，核心零改动）。但真正承诺批产前，
  还要看 **#10 公司**：它是第三套 `Era` 叙事 + 第二套立场轴，且不做任何新引擎能力。
