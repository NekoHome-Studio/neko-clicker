...
撞车 0、倒挂 0、已读集合无空洞（其余 8 条是转生 4~5 级的后盘内容）。
...
- 第 5 命 `peak_cps` 门槛造成约 19 小时节奏凸起，与相邻层 2~4 小时不成比例。属于内容调参，不影响机制。
- 咖啡馆后盘 8 条要求转生 4~5 级 + 单轮 1e14~3e14，是否过苦工待试玩判断（原设计 `door_20` 要 1e15，已下调）。
- `LoreTests.FirstThreshold` 取"条件里最小的数字叶子"，会把 `EraAtLeast(2)` 里的层号 2 误当成早期门槛。
  当前各包都还能通过（另有真正早期的开篇条目撑着），但这条助手已经不再有判别力，值得修。
