...
| G6 | 每次改动后既有测试全绿（当前 277 个） | `.\tools\build.ps1` 退出码 0 |
...
S-A / S-B / S-C 三项引擎能力也随之落地；阶段 4~5 按需追加。
...
- 阶段 0（#1 猫娘咖啡馆 + 架构测试 + `--package`）、阶段 1（S-A `Era` + #2 九命轮回）、
  ...
  立场轴 + 终局判定 + 九命四条立场）、3C-1（#3 实验室）与 3C-2（#10 公司）均已落地。
  十个包里 #1 / #2 / #3 / #10 已可玩，示例包「猫咖物语」作为回归基线保留。
  **S-D `Decay` 一行未写**；#4~#9 仍只是文档。
...
- 阶段 5 的四个"换皮"包能批量做，前提是阶段 1~2 的抽象确实成立——**该前提已由 #3 与 #10
  ...
  两套立场轴，核心零改动）。批量生产的能力条件已经具备，剩下的成本在文案（见 K4）。
