...
| 项 | 内容 |
|---|---|
| 交付 | S-B 全部接口 + #1 咖啡馆 50 条（3 条线）+ #2 九命 46 条（4 条线）+ 图鉴面板 |
...
- 第 5 命 `peak_cps` 门槛造成约 19 小时节奏凸起，与相邻层 2~4 小时不成比例。
  属于内容调参，不影响机制。
...
| K4 | 叙事量大（十包约 600 条）拖垮进度 | 高 | 先只做 #1 的 50 条，测写作速度 | 只承诺前 3 个包，其余按需；**阶段 2 实测**：两包 96 条（#1 50 + #2 46），写作速度可控，但 #2 的条目数少于 #1 是因为叙事更多落在纪元文字上 |
