...
#### 4B —— S-D（Decay）+ #9 图书馆 ⬜ 待做

接口**有意留白**（见 §6.5）：虚无化的具体形式（线性衰减 / 阈值悬崖 / 有无数值下限）
必须由 #9 包的实际节奏反推，现在设计等于拍脑袋。4B 开工前要先写 #9 的内容规格。
...
- 阶段 0（#1 猫娘咖啡馆 + 架构测试 + `--package`）、阶段 1（S-A `Era` + #2 九命轮回）、
  ...
  立场轴 + 终局判定 + 九命四条立场）、3C-1（#3 实验室）、3C-2（#10 公司）与 4A
  （跨转生继承 + #6 末世）均已落地。
  十个包里 #1 / #2 / #3 / #6 / #10 已可玩，示例包「猫咖物语」作为回归基线保留。
  **S-D `Decay` 一行未写**；#4 / #5 / #7 / #8 / #9 仍只是文档。
- 4A 的继承**只用了 `EraDefinition` 上阶段 1 就位的两个字段**，核心零改动；
  它验证的是"既有能力真的够用"，不是"新写了什么"。
- 阶段 4 的 S-D 接口**有意留白**，因为它的正确形式依赖 #9 包的实际节奏，现在设计等于猜。
- 四个纪元包的「转生货币线」在当前参数下自然游玩打不开（详见 4A 末尾的跨包观察）。
